  ...
  cron.schedule("* * * * *", async () => {
    try {
      ...
      for (const subscription of subscriptions) {
        const { _id, fcmToken, auctionId, whatsappNumber, email } =
          subscription;

        if (scheduledNotifications.has(_id.toString())) {
          console.log(
            `⏩ Notification for auction ${auctionId} already scheduled. Skipping.`
          );
          continue;
        }
        ...
        if (!auctionStartTime || isNaN(auctionStartTime.getTime())) {
          console.error(
            `🚫 Invalid or missing start time for auction ID ${auctionId}. Skipping.`
          );
          continue;
        }
        ...
        if (
          ...
          if (fcmToken) {
            ...
            console.log(
              `✅ Push notification sent for auction ${auctionId} - Product: ${productName}`
            );
          }
          if (whatsappNumber) {
            const productName = auction.itemTitle || "Unknown Product";
            ...
            console.log(
              `✅ WhatsApp message sent to ${whatsappNumber} for auction ${auctionId}`
            );
          }
          ...
          console.log(
            `⏲️ Notification time not yet reached for auction ${auctionId}. Current time (ISO): ${nowISO}`
          );
        }
      }
      ...
      console.error(
        "❌ Error encountered in cron job for auction notifications:",
        error
      );
    }
  });
